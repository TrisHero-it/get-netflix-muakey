...
class Account extends db
{
    ...
    {
        $query = "SELECT accounts.id, accounts.account_id, accounts.category_id, categories.name, accounts.code, accounts.created_at, accounts.email, accounts.password, accounts.code_2fa, accounts.user, accounts.pin_code, accounts.expired_at FROM accounts join categories on accounts.category_id = categories.id WHERE account_id = $accountId ORDER BY id DESC LIMIT 1";
        return $this->getData($query, false);
    }
    ...
    public function insert2($email, $password, $code2fa, $code, $category, $user, $shipment = 1, $pin = null, $account_id = null, $expired_at = null)
    {
        $query = "INSERT INTO accounts (email, password, code_2fa, code, category_id, user, shipments, pin_code, account_id, expired_at) 
        VALUES ('$email', '$password', $code2fa , $code, '$category', '$user', '$shipment', $pin, $account_id, $expired_at)";
        $this->getData($query, false);
    ...
    {
        $query = "SELECT accounts.id, accounts.expired_at , accounts.account_id, accounts.category_id, categories.name, accounts.code, accounts.created_at, accounts.email, accounts.password, accounts.code_2fa, accounts.user, accounts.pin_code FROM accounts join categories on accounts.category_id = categories.id WHERE accounts.code = '$code'";
        return $this->getData($query, false);
    }
    ...
    public function updateAccount($id, $email, $password, $code2fa, $category, $user, $pin = null, $expired_at = null)
    {
        $query = "UPDATE accounts 
                  SET email = '$email', password = '$password', code_2fa = $code2fa, category_id = '$category', user = '$user', pin_code = $pin, expired_at = $expired_at 
                  WHERE id = $id";
        $this->getData($query, false);
    }
